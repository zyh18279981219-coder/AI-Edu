...
| 学生数字孪生 | 汇总测验、作业、代码题、资源学习、5E 交互证据，形成知识点掌握、章节综合实践能力、职业能力达成和学生端/教师端不同视图。 | 部分实现。已有画像总表、知识点画像、掌握度公式和画像摘要；章节综合实践能力基础计算与学生端展示已接入，教师确认的作业覆盖知识点已作为叶子知识点辅助证据进入画像摘要和诊断统计；职业能力达成展示、5E 证据仍待补齐。 | 继续补职业能力达成等级、5E 证据回流和教师端差异化证据展示。 |
...
| 作业与实践评测 | 稳定支持主观题、客观题、选择题、代码题；章节作业默认作为章节综合实践证据，覆盖知识点为教师确认的可选配置。 | 部分实现。作业创建、提交、评分已跑通；覆盖知识点配置表、读取和更新接口已实现；章节主观题/代码题已聚合为学生画像中的章节综合实践能力，教师确认覆盖知识点已进入诊断作业证据统计。 | 下一步补作业证据在教师看板中的贡献展示和代码题稳定性专项验证。 |
...
当前基础版已接入章节综合实践能力：章节主观题和代码题作为章节综合实践证据，代码题权重高于普通主观题；如果教师确认作业覆盖知识点，则该作业结果作为对应叶子知识点的辅助证据进入学生画像摘要和诊断统计，但不替代在线测验证据。
...
2. 作业覆盖知识点、章节综合实践能力、资源学习事件、路径节点状态、诊断人工修正、干预任务包已接入基础业务链路；下一步继续补职业能力达成等级、教师看板统一诊断读取、干预回流和 5E 有效性事件。
...
| 作业覆盖知识点和章节实践能力仍需产品化 | 基础聚合与学生端展示已接入，但教师看板贡献展示、修正入口和演示数据仍不足 | 下一步补教师看板作业证据贡献展示、稳定演示数据和更细 smoke/pytest |
