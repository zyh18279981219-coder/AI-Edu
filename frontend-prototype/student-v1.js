...
// ===================================================
// V1 专用：学习中心 - 5E智能体 + 知识图谱面板
// ===================================================

// 5E阶段切换
function setFiveEStage(stage) {
    const stageOrder = ['engagement', 'exploration', 'explanation', 'elaboration', 'evaluation'];
    const steps = document.querySelectorAll('.five-e-step');
...
// 在对话区显示资源
function showResourceInChat(type, name) {
    const messagesContainer = document.getElementById('messages');
    if (!messagesContainer) return;
    
    // 添加用户消息
    appendUserMsg(`查看${name}的${type}`);
            ...
            const pdfHTML = `
                <div class="msg agent-msg">
                    ...
                    <div class="msg-bubble" style="max-width: 100%;">
                        <p>已为你打开「${name}」的PDF文档：</p>
                        <div class="resource-preview-card">
                            <div class="preview-header">
                                <span class="preview-icon">📄</span>
                                <span class="preview-title">${name}.pdf</span>
                                <span class="preview-pages">15页</span>
                            </div>
                            <div class="preview-content">
                                <div class="preview-page">
                                    <h3>第1页 - ${name}</h3>
                                    <p>这是PDF文档的预览内容。在实际系统中，这里会显示PDF的实际内容。</p>
                                    <p>学生可以在这里阅读文档，并随时向智能体提问。</p>
                                    <div class="preview-placeholder">
                                        <div class="placeholder-icon">📖</div>
                                        <div class="placeholder-text">PDF内容预览区域</div>
                                    </div>
                                </div>
                            </div>
                            <div class="preview-actions">
                                <button class="preview-btn" onclick="alert('下载功能')">⬇️ 下载</button>
                                <button class="preview-btn" onclick="alert('全屏查看')">🔍 全屏</button>
                                <button class="preview-btn" onclick="alert('打印功能')">🖨️ 打印</button>
                            </div>
                        </div>
                        <p style="margin-top: 12px;">阅读完后，有什么问题可以随时问我 😊</p>
                    </div>
                </div>
            `;
            messagesContainer.innerHTML += pdfHTML;
        } else if (type === '视频') {
            const videoHTML = `
                <div class="msg agent-msg">
                    <div class="msg-avatar">🤖</div>
                    <div class="msg-bubble" style="max-width: 100%;">
                        <p>已为你打开「${name}」的视频讲解：</p>
                        <div class="resource-preview-card">
                            <div class="preview-header">
                                <span class="preview-icon">🎥</span>
                                <span class="preview-title">${name}讲解视频</span>
                                <span class="preview-duration">12:35</span>
                            </div>
                            <div class="preview-video">
                                <div class="video-placeholder">
                                    ...
                                    <div class="video-info">
                                        ...
                                        <div class="video-meta">高清 · 12分35秒</div>
                                    </div>
                                </div>
                            </div>
                            <div class="preview-actions">
                                <button class="preview-btn" onclick="alert('播放视频')">▶️ 播放</button>
                                <button class="preview-btn" onclick="alert('倍速设置')">⚡ 倍速</button>
                                <button class="preview-btn" onclick="alert('字幕设置')">💬 字幕</button>
                            </div>
                        </div>
                        <p style="margin-top: 12px;">观看完后，我们继续学习 😊</p>
                    </div>
                </div>
            `;
            messagesContainer.innerHTML += videoHTML;
        }
        scrollMessages();
    }, 500);
}
...
function jumpToNode(nodeName) {
    appendUserMsg(`我想学习「${nodeName}」`);
    setTimeout(() => {
        appendAgentMsg(`好的！正在为你切换到「${nodeName}」，请稍等...`);
        setTimeout(() => {
            appendAgentMsg(`已切换到「${nodeName}」。让我们开始学习吧！`);
        }, 1000);
    }, 500);
}
...
document.addEventListener('DOMContentLoaded', function() {
    // 章节折叠
    document.querySelectorAll('.chapter-header').forEach(hd => {
    ...
    // V1输入框：Enter发送
    const v1Input = document.getElementById('chat-input');
    if (v1Input) {
...
function appendUserMsg(text) {
    ...
    div.innerHTML = `<div class="msg-bubble"><p>${escapeHtml(text)}</p></div><div class="msg-avatar">👤</div>`;
...
function appendAgentMsg(html) {
    ...
    div.innerHTML = `<div class="msg-avatar">🤖</div><div class="msg-bubble"><p>${html}</p></div>`;
    container.appendChild(div);
}
