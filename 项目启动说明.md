...
为避免每位开发者本地 SQLite 数据不一致，建议先完成一次初始化。

### 5.1 导出当前数据库为原始种子（维护者执行）
...
- 该命令会把当前数据库完整导出到 `release/init_seed.sql`
- 后续团队成员可基于这份 SQL 得到一致初始数据

### 5.2 从原始种子导入本地数据库（团队成员执行）
...
- 导入前会默认备份已有库到 `data/app.db.bak`
- 导入后本地库会重建为统一版本

### 5.3 仅补齐现有 teacher 账号的雷达图数据
...
- 该脚本只处理当前数据库中的 teacher 账号，不会新增学生账号
- 用于补齐教师六维雷达图所需日志与扩展指标
- 默认不会覆盖已存在的学生 `twin_profiles`，只补缺失数据
- 如需强制覆盖学生画像，可加参数：`--overwrite-student-twins`
