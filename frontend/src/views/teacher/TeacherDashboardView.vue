        ...
        <article v-if="selectedStudentDetail" class="card-panel teacher-detail-card">
          ...
          <div class="card-panel teacher-weak-card">
            <div class="section-head">
              <div>
                <h3>统一诊断结论</h3>
                <p class="hero-desc">{{ selectedStudentDetail.diagnosis?.persona_summary || '基于学生画像和近期学习证据识别薄弱知识点。' }}</p>
              </div>
              <div v-if="selectedStudentDetail.diagnosis" class="teacher-diagnosis-badges">
                <span class="relevance-pill" :class="evidenceLevelClass(selectedStudentDetail.diagnosis.evidence_level)">
                  {{ evidenceLevelLabel(selectedStudentDetail.diagnosis.evidence_level) }}
                </span>
                <span class="relevance-pill mastery-mid">
                  置信度 {{ selectedStudentDetail.diagnosis.confidence ?? 0 }}%
                </span>
              </div>
            </div>
            <ul class="message-list">
              <li v-for="node in selectedStudentDetail.weak_nodes" :key="`${node.node_id}-${node.priority}`">
                <div class="teacher-weak-node-row">
                  <div>
                    <strong>{{ node.node_id }}</strong>：掌握度 {{ node.mastery_score }}%，优先级 {{ node.priority }}
                    <span v-if="node.confidence != null">，置信度 {{ node.confidence }}%</span>
                  </div>
                  <span v-if="node.evidence_level" class="relevance-pill" :class="evidenceLevelClass(node.evidence_level)">
                    {{ evidenceLevelLabel(node.evidence_level) }}
                  </span>
                </div>
                <p v-if="node.teacher_explanation" class="teacher-node-explanation">{{ node.teacher_explanation }}</p>
                <div v-if="node.suggested_actions?.length" class="teacher-action-chips">
                  <span v-for="action in node.suggested_actions" :key="`${node.node_id}-${action}`">{{ suggestedActionLabel(action) }}</span>
                </div>
              </li>
          ...
          <div v-if="selectedStudentDetail.diagnosis?.evidence_timeline?.length" class="card-panel teacher-weak-card">
            <div class="section-head">
              <h3>诊断证据时间线</h3>
              <span class="muted">最近 {{ selectedStudentDetail.diagnosis.evidence_timeline.length }} 条证据</span>
            </div>
            <div class="teacher-evidence-timeline">
              <article
                v-for="item in selectedStudentDetail.diagnosis.evidence_timeline.slice(0, 8)"
                :key="`${item.type}-${item.node_id}-${item.occurred_at}-${item.title}`"
                class="teacher-evidence-item"
              >
                <div class="teacher-evidence-top">
                  <span class="relevance-pill mastery-mid">{{ evidenceTypeLabel(item.type) }}</span>
                  <span class="muted">{{ formatEvidenceTime(item.occurred_at) }}</span>
                </div>
                <strong>{{ item.node_id || item.title || item.resource_path || '学习证据' }}</strong>
                <p>{{ evidenceItemSummary(item) }}</p>
              </article>
            </div>
          </div>
        </article>
...
function evidenceLevelClass(level?: string) {
  if (level === "sufficient") return "mastery-high";
  if (level === "partial") return "mastery-mid";
  return "mastery-low";
}
...
function suggestedActionLabel(action: string) {
  const labels: Record<string, string> = {
    take_quiz: "补充测验",
    collect_learning_evidence: "采集学习证据",
    review_resource: "复习资源",
    assign_practice: "安排练习",
    teacher_feedback: "教师反馈",
    schedule_code_practice: "安排代码实践",
    explain_misconception: "讲解误区",
  };
...
function evidenceItemSummary(item: {
  type?: string;
  score?: number;
  total?: number;
  passed?: boolean;
  ...
  if (item.type === "quiz") {
    ...
    return `得分 ${item.score ?? "-"}${total}，${item.passed ? "已通过" : "需继续巩固"}`;
  }
  if (item.type === "homework") {
    const score = item.score != null ? `得分 ${item.score}` : `状态 ${item.status || "-"}`;
    return `${item.title || "作业"}，${score}`;
  }
