...
class DataCollector:
    ...
    def _resolve_student_username(self, identifier: str) -> str | None:
        user = self.database_store.get_user_by_identifier("student", identifier)
    ...
    def collect_llm_interactions(self, username: str) -> None:
        ...
        if not username:
            return
        try:
            raw_logs: list[dict] = self.database_store.list_llm_logs_for_user(username, user_type="student")
        except Exception:
            raw_logs = []
    ...
    def collect_session_duration(self, username: str) -> None:
        ...
        try:
            user_sessions = self.database_store.list_sessions_for_user("student", username)
        except Exception:
            user_sessions = []
