...
from HomeworkModule.models import (
    ...
    AssignmentCoverageUpdateRequest,
    AIGradeRequest,
...
def _require_teacher(session_id: Optional[str]) -> Dict[str, Any]:
    ...
    if not database_store.get_user("teacher", session.get("username", "")):
...
def _require_student(session_id: Optional[str]) -> Dict[str, Any]:
    ...
    if not database_store.get_user("student", session.get("username", "")):
...
def _require_teacher_or_admin(session_id: Optional[str]) -> Dict[str, Any]:
    ...
    if user_type == "teacher":
        ...
        return session
    if user_type == "admin":
        if not database_store.get_user("admin", username):
...
def publish_assignment(data: AssignmentCreateRequest, session_id: Optional[str] = Cookie(None)):
    ...
    payload = {
        ...
        "covered_knowledge_points": [item.model_dump() for item in data.covered_knowledge_points],
...
@router.get("/assignments/{assignment_id}/coverage")
def get_assignment_coverage(assignment_id: str, session_id: Optional[str] = Cookie(None)):
    session = _require_teacher_or_admin(session_id)
    assignment = service.get_assignment(assignment_id)
    if not assignment:
        raise HTTPException(status_code=404, detail="作业不存在")
    if session.get("user_type") == "teacher" and assignment.get("created_by") != session.get("username"):
        raise HTTPException(status_code=403, detail="无权查看该作业覆盖知识点")
    return {
        "success": True,
        "assignment_id": assignment_id,
        "covered_knowledge_points": service.list_assignment_coverage(assignment_id),
    }


@router.put("/assignments/{assignment_id}/coverage")
def update_assignment_coverage(
    assignment_id: str,
    data: AssignmentCoverageUpdateRequest,
    session_id: Optional[str] = Cookie(None),
):
    session = _require_teacher(session_id)
    assignment = service.get_assignment(assignment_id)
    if not assignment:
        raise HTTPException(status_code=404, detail="作业不存在")
    if assignment.get("created_by") != session.get("username"):
        raise HTTPException(status_code=403, detail="无权修改该作业覆盖知识点")
    coverage = service.replace_assignment_coverage(
        assignment_id,
        [item.model_dump() for item in data.covered_knowledge_points],
        teacher_username=str(session.get("username") or ""),
    )
    return {"success": True, "assignment_id": assignment_id, "covered_knowledge_points": coverage}
