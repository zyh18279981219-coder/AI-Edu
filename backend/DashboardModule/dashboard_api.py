...
_database_store = get_database_store()
_teacher_twin_service = TeacherTwinService()
_diagnosis_service = StudentDiagnosisService()
...
def get_student_detail(username: str, session=Depends(_require_teacher)):
    ...
    try:
        diagnosis = _diagnosis_service.generate_student_diagnosis(username, persist=False)
    except Exception:
        diagnosis = None

    if diagnosis:
        weak_nodes = []
        for idx, item in enumerate(diagnosis.get("weak_nodes") or [], start=1):
            if not isinstance(item, dict):
                continue
            weak_nodes.append({**item, "priority": idx})
        ...
        result["diagnosis"] = {
            ...
            "evidence_timeline": (diagnosis.get("teacher_view") or {}).get("evidence_timeline", []),
            "manual_correction_supported": (diagnosis.get("teacher_view") or {}).get(
